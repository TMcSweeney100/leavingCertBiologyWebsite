...
| | 2F Personal items and timeline | written 16 Sep 2026 | **built 18 Sep 2026**; `make verify` green (348 backend tests; 110 `node:test`, 129 Vitest specs); `PersonalItemPrivacyTest` proves no teacher, leader or other student reaches a student's personal items through any endpoint, with a mutation-and-revert bite test; Task 7 restyle skipped — `docs/design/pilot/D-4-timeline/` doesn't exist yet (see HANDOFF) | **Gate P2** |
...
**Status, 18 Sep 2026:** the four final 2027 briefs are in `subjectDocs/` (2B Task 1 renames them to their SEC codes). The Coursework Rules and Procedures is still missing; Phase 2 doesn't need it, Phase 6 does (Q6). The D-3, D-4 and D-5 prompts are written, the packs haven't arrived; each page plan builds working-first and ends with a restyle task that waits for its pack. **All six plans are written** (table at the top). Every content string in 2B and 2C, and the SQL for V6–V10 and the timeline query, was run on Postgres 18 and checked against the PDFs before the plans were saved. **2A–2F are all built — Phase 2's code and tests are done.** What's left of Gate P2 is human review (Tim's content check, Katelyn's Biology review) and the design packs, none of which is code (see the Gate P2 checklist above and `docs/HANDOFF.md`).
...
| **2C Business content and 2027 briefs** | Business template (6 stages plus Compilation, `hours_group` for 4–5). Four `annual_brief` rows with rules, word/image limits and completion dates from the **final** briefs, never the `EN-EX` samples. Test: each brief's completion date equals the §4.3 table. **Built 18 Sep 2026.** Plan: `docs/superpowers/plans/2026-09-16-pilot-2c-business-content-and-briefs.md`. **Tim's content review against the PDFs is still open** (Gate P2). |
...
| **2E Student component page** | `GET /components/{id}` shaped by role. `item_tick`. App components for the page (not the BiPi components' props, which need teacher-written text the template doesn't hold; Q-P2-C, 16 Sep 2026) — the stage state comes from the class's own dates, reusing `lib/schedule.ts`'s countdown and Dublin-safe date logic. D-3 decides the look. Page `/components/[id]`. **Built 18 Sep 2026.** Plan: `docs/superpowers/plans/2026-09-16-pilot-2e-student-component-page.md`. Task 8 restyle skipped — D-3 hasn't arrived. |
| **2F Personal items and timeline** | `personal_item` (owner-only; authz test that teacher and leader sessions can't reach any row). Timeline query as a union with `from`/`to`. `/home` list, week and month views, add/edit/delete. **Built 18 Sep 2026.** Plan: `docs/superpowers/plans/2026-09-16-pilot-2f-personal-items-and-timeline.md`. Task 7 restyle skipped — D-4 hasn't arrived. |

**Gate P2**
- [x] `make verify` green: 348 backend tests; 110 `node:test`, 129 Vitest specs, lint, types, build — code and tests for the whole phase are green
- [ ] `make e2e` confirmed green on this branch with the extended journey (teacher creates a Biology component, sets dates, one date after the completion date is refused with the named message; student sees the stages and adds a personal item; the timeline shows both, in order, labelled) — the run was started separately from this session; see `docs/HANDOFF.md` for its result
- [ ] Content: Tim has checked every checkpoint, prompt, rule, band and date against the source PDFs — **still open**, not done in 2B or 2C; see `docs/HANDOFF.md` "Waiting on a human"
- [ ] Katelyn has reviewed the Biology checkpoints — **still open**; see `docs/HANDOFF.md` "Waiting on a human"
