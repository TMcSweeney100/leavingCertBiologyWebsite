...
- School identity at the top of the header: a school's crest (optional — some classes have
  none, e.g. the demo class) beside the school name, and the class teacher. The name and
  teacher are text, not part of the image, and live in that class's own file under
  `lib/classes/` (Phase B; see the "Schedule data location" entry below)
...
- **Schedule data location (Phase B — replaces the single `schedule.data.ts` file):** content
  ...
  20 lines — and `frontend/lib/classes/index.ts` registers it (`CLASSES`, `CLASS_SLUGS`,
  `getClass`). `frontend/lib/class-resolve.ts` (`resolveClass`) merges a class with its brief
  into the `ResolvedClass` the page renders; `frontend/lib/schedule.types.ts` holds the shared
  types (`Brief`, `ClassConfig`, `ResolvedClass`, `Stage`, `ReportSectionRow`, …). **Adding a
  teacher is a new file in `lib/classes/` plus one line in `lib/classes/index.ts` — nothing
  else changes.**
- **Date logic location:** `frontend/lib/schedule.ts` (`deriveSchedule(cls, now)`, `dublinToday`,
  `parsePreviewDate`, `formatTodayLabel`, `countdownText`/`daysLeftWord`, `DUE_TODAY`,
  `termPositionPct(iso, term)`, `termSpanLabel(term)`), tested in `frontend/lib/schedule.test.ts`
  against the implementation plan's §3.3 table (Phase B threads a resolved class through every
  call there; no expected values changed). Implements the Dublin-timezone-safe algorithm from
  plan §3.2. Every consumer must call through `deriveSchedule`/`dublinToday` with an injected
  `now`, never compute the current date independently.
...
- **`ReportSectionRow` (and every other schedule type) lives in `lib/schedule.types.ts`** (Phase
  B), not beside the logic in `schedule.ts` or inside a data file — it's shared by the brief,
  the resolver and the derivation logic, so it needs one home all three can import without a
  cycle. It still infers as a union at its source (`biology-2027.ts`'s `reportSections`: rows
  with `alwaysDone`, rows with `stageId`) and is widened the same way as before.
...
- **`lib/site.ts` holds this site's configuration: `NEXT_PUBLIC_SITE_URL` and, since Phase B,
  `NEXT_PUBLIC_DEFAULT_CLASS`.** `SITE_URL`/`hasSiteUrl`/`SITE_URL_LABEL` are read from the
  former. It is **empty by default and everything URL-dependent renders nothing until it is
  set**: the noticeboard QR block is absent rather than pointing at a placeholder domain, and
  `metadataBase` is omitted (which costs exactly one expected build warning about link-preview
  images falling back to localhost — setting the variable removes it). Set it in the Vercel
  project's environment variables, no trailing slash, then redeploy; nothing else changes.
  `DEFAULT_CLASS` is unrelated and optional: it just picks which class `/` redirects to, falling
  back to the first registered class (`CLASS_SLUGS[0]`) when unset.
