...
- **UI component location:** `frontend/components/ui/` (shadcn primitives — `button.tsx`, `progress.tsx`) and `frontend/components/bipi/` (app-specific components, Phase 3+). So far: `site-header.tsx`, `site-footer.tsx`, `stage-card.tsx`, `timeline.tsx`, `you-are-here.tsx`, `term-progress.tsx`, `sticky-now-bar.tsx`. **Every file under `components/bipi/` is a server component** and should stay one — per plan §4.3, `stage-card.tsx` (Phase 7) is meant to be the only one that ever carries `"use client"`. The boundary is subtler than §4.3 states, though: shadcn's `progress.tsx` carries `"use client"` itself (Base UI's Progress is a client component), so `term-progress.tsx` is a server component *rendering* a client primitive. That is what decision #5 asks for and is fine; what matters is that no `components/bipi/` file gets the directive.
- **shadcn `Progress` needs `locale="en-IE"`.** Base UI formats the percentage — both the visible `ProgressValue` text and the root's `aria-valuetext` — with the ambient default locale, which is the server's on the server and the browser's on the client. Left unpinned that is a hydration mismatch waiting for the first visitor whose locale renders "34 %". `components/ui/progress.tsx` also carries one local addition to the generated file: `trackClassName`/`indicatorClassName` pass-throughs, because the composite renders those two parts itself and the design needs two different tracks (6px mobile bar, 2px laptop ruler) off one primitive. Everything else in that file is registry output; keep it that way.
...
- **Nav anchor-id convention (fixed, from `HEADER.nav`, kebab-case):** `#right-now` (the "you are here" panel, built) and `#timeline` (the timeline section heading, built) exist; `#report-sections` (Phase 8 crosswalk heading) and `#report-rules` (Phase 8 report-rules heading) do not yet. `site-header.tsx` emits all four links (`NAV_IDS` constant, top of file). Give each remaining target that literal `id` plus `scroll-mt-10 lg:scroll-mt-6` — both sit below the sticky mini-banner, so mobile has to clear its 29px, while `lg` has nothing sticky above it and only wants a little breathing room.
