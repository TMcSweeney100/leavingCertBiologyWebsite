...
- **Component-scoped endpoints go through `ComponentService.owned` (the class's owner) first.** It requires TEACHER, finds the component by id, and separately confirms the class it belongs to is owned by the actor; either failing is `NOT_FOUND`.
- **A teacher item id is only ever looked up with `TeacherItemRepository.findActive(itemId, componentId)`**, never by id alone, so an id from another component is not found and a retired item is invisible.
- Students see only their own enrolments (`EnrolmentService.myClasses(actor)`). School leader scope arrives in Phase 5.
- **A student reaches a component only through `ComponentRepository.findForApprovedStudent(componentId, studentId)`.** It joins to `enrolment` and requires `status = 'APPROVED'`, so a pending or removed student, or a student of another class, gets `NOT_FOUND` the same as a stranger. `ComponentService.view` tries the teacher's `owned` first and falls back to this for everyone else; `ComponentService.tick` calls it before touching `item_tick`.
...
- **Personal items are private to their owner.** Every `PersonalItemRepository` method (`ie.coursework.timeline.adapter.persistence`) takes the owner and puts it in its `WHERE`; there is deliberately no lookup by id alone, so a wrong or stale id from another student's item just isn't found. No other feature joins `personal_item`. `PersonalItemPrivacyTest` (`timeline/authz/`) proves it end to end: a teacher, a school leader and another student all get 404 from every `/me/personal-items` endpoint for an id that isn't theirs, including a mutate-then-revert bite test that a naive "any row with this id" query would pass by accident.
- `TimelineService.edit()` only re-checks a personal item's class label (`checkClass`) when the submitted `classId` actually differs from the item's stored one, not on every edit. The first version re-validated on every save, which meant a student removed from a class could no longer edit *any* field of an item they had labelled with that class while still enrolled — the fix (found in review, commit `7244574`) is the general pattern for this kind of "label set once, checked again on save" field: only re-run the check when that field's value is the thing changing.
...
- **`components` feature (`ie.coursework.components`), app schema `V8__component_instances.sql`**: `component_instance` (one per `class_group_id`, unique; pins an `annual_brief_id`), `instance_stage_date` (a stage's due date; no row means no date set yet, dates needn't be in stage order), `teacher_item` (a teacher's own to-dos inside a stage, `ordinal` within the stage, retired by timestamp, never deleted). A trigger refuses inserting a stage date or a teacher item's due date after the component's brief's completion date (`check_violation`, the same error the service reports as `COMPLETION_DATE_EXCEEDED`), and refuses a stage id from a different template version than the component's brief. `ComponentService`/`ComponentController` list published briefs, create a component, return a role-shaped view (§10), save stage dates as a batch, and add/edit/retire teacher items. These are app tables, not content: truncated before each test like `class_group`.
- **`V9__item_ticks.sql`**: `item_tick` (primary key `student_user_id, teacher_item_id`; unticking sets `done_at` back to null rather than deleting the row, so the student's history of touching an item survives). `ItemTickRepository.doneItems`/`set` are keyed on student and component, not enrolment, because an enrolment can be re-created. `ComponentService.studentView` reads it alongside `TemplateRepository`'s new `sections`/`bands`/`prompts`/`checkpoints`/`processNote` and `BriefRepository`'s new `details`/`rules` to build the student's role-shaped view (§10); `CheckpointState.at(stageDate, today)` (`components/domain/`) is `DUE` once `today` is strictly after the stage's date, `NOT_DUE` otherwise or with no date, and `DublinDate.today(clock)` reads the one `Clock` bean in `Europe/Dublin` rather than the server's UTC day.
- **`timeline` feature (`ie.coursework.timeline`), `V10__personal_items.sql`**: `personal_item` (owner `student_user_id`, `title`, `due_date`, `kind`, an optional `class_group_id` label). `TimelineRepository.between` is one union query — approved-enrolment stage dates, approved-enrolment dated active teacher items, and the student's own personal items, each branch filtered to the `[from, to]` range — ordered by date, then kind (stage before teacher item before personal), then subject and stage/item ordinal. `TimelineRange.problem(from, to)` (`timeline/domain/`) rejects `to` before `from` and caps any range at 100 days (`TimelineRange.MAX_DAYS`), so nobody can ask the union query to scan the whole table. `TimelineService.add`/`edit` label an item with a class only when the student has actually joined it (`checkClass`, via `EnrolmentRepository.findByStudent`, excluding `REMOVED`); `list`/`find`/`update`/`delete` all go through `PersonalItemRepository` keyed on the owner (see §4's personal-item bullet).
...
- **The component page** (`app/(app)/components/[id]/page.tsx`) is a role-shaped load on top of the same pattern: `serverApi.get` parses the response against `componentViewSchema`, a discriminated union on `view`. `"STUDENT"` renders `ComponentOverview`; `"TEACHER"` redirects to that class's Component tab, because `/components/[id]` isn't a teacher page (P2-41). `ComponentOverview` derives stage state from `lib/app/component-progress.ts`'s pure `progress()`, never from `Date.now()` (see below), and renders `ItemTick` client components for the teacher's items, each ticked with `api.send` then `router.refresh()`.
- **`/home` (`app/(app)/home/page.tsx`) is the student's timeline, range from the URL.** `lib/app/timeline.ts`'s `rangeFor(view, anchor)` turns `?view=list|week|month&from=<date>` into a `{from, to}` pair (list defaults to today onward, capped like the backend; week and month are calendar ranges built from pure UTC-midnight date-string arithmetic — `stepFrom`, `monthWeeks`, `weekday` — so there's no timezone or DST risk); the page reads those two query params, calls `serverApi.get('/me/timeline?from=&to=', timelineSchema)` inside `attempt()`, and renders `TimelineView` with the merged, ordered entries (stage dates, teacher items, personal items) plus `AddPersonalItem`/`PersonalItemActions` client components for add/edit/delete. Previous/next links change only `from` (and `view` when switching list/week/month), so the range is shareable and back/forward just works — no client-side range state to keep in sync with the URL.
...
- `ComponentController.view` returns `Object`, not the `ComponentViews.ComponentView` interface every role's response implements: Jackson serialises by the declared static type, so returning the interface would write only its one `view` property and drop the runtime record's (`TeacherComponent`, now also `StudentComponent`).
- **The student view carries `today`: the page derives the current stage from the server's Dublin date, never the browser's clock.** `DublinDate.today(clock)` computes it server-side and `component-progress.ts`'s `progress()` takes it as a plain string argument, so a page rendered just after midnight in Dublin can't disagree with the checkpoint states (`CheckpointState.at`) computed from that same `today` in the same response.
