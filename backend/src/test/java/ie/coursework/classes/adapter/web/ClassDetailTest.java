...
class ClassDetailTest extends PostgresIntegrationTest {
    ...
    void rotatingReplacesTheCodeAndIsAudited() throws Exception {
        ...
        teacher.post("/api/v1/classes/" + world.class1() + "/join-code")
                .andExpect(status().isOk())
    ...
    @Test
    void rotatingNeverWritesTheCodeToTheAuditLog() throws Exception {
        ClassFixtures.World world = fixtures.world();
        ApiSession teacher = teacher1();

        String response = teacher.post("/api/v1/classes/" + world.class1() + "/join-code")
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
        String newCode = JsonPath.read(response, "$.code");

        String details = jdbcTemplate.queryForObject(
                "SELECT details::text FROM audit_event WHERE event_type = 'JOIN_CODE_ROTATED' AND subject_id = ?",
                String.class, world.class1());
        assertThat(details).as("the code itself is never written to the audit log")
                .doesNotContain(newCode)
                .as("no incidental payload is written either")
                .isEqualTo("{}");
    }
